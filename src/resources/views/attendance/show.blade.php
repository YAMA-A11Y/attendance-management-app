...
<div class="attendance-detail-page">
    <div class="attendance-detail-container">
        ...
        <form action="{{ route('attendance.correction_request.store', ['id' => $attendance->id]) }}" method="POST" class="attendance-detail-form">
            @csrf
            ...
            @php
                $displayClockInAt = old(
                    'clock_in_at',
                    $pendingCorrectionRequest && $pendingCorrectionRequest->requested_clock_in_at ? \Carbon\Carbon::parse($pendingCorrectionRequest->requested_clock_in_at)->format('H:i') : ($attendance->clock_in_at ? \Carbon\Carbon::parse($attendance->clock_in_at)->format('H:i') : '')
                );

                $displayClockOutAt = old(
                    'clock_out_at',
                    $pendingCorrectionRequest && $pendingCorrectionRequest->requested_clock_out_at ? \Carbon\Carbon::parse($pendingCorrectionRequest->requested_clock_out_at)->format('H:i') : ($attendance->clock_out_at ? \Carbon\Carbon::parse($attendance->clock_out_at)->format('H:i') : '')
                );
                ...
                $breakTimes = $pendingCorrectionRequest ? $pendingCorrectionRequest->breakTimes : ($attendance->breakTimes ?? collect());
                    ...
                    <div class="attendance-detail-row">
                        ...
                        <div class="attendance-detail-row__value attendance-detail-row__value--time-range">
                            <input
                                ...
                                class="attendance-detail-time-input"
                                value="{{ old('breaks.' . $i . '.break_start_at', $break && $break->break_start_at ? \Carbon\Carbon::parse($break->break_start_at)->format('H:i') : '') }}"
                            >
                            <span class="attendance-detail-time-separator">〜</span>
                            <input
                                type="time"
                                ...
                                class="attendance-detail-time-input"
                                value="{{ old('breaks.' . $i . '.break_end_at', $break && $break->break_end_at ? \Carbon\Carbon::parse($break->break_end_at)->format('H:i') : '') }}"
                            >
                        </div>
                    </div>
                @endfor
            ...
            @else
                <div class="attendance-detail-button-area">
                    <button type="submit" class="attendance-detail-button">修正</button>
                </div>
            @endif
        </form>
    </div>
